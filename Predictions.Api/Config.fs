...
module Config =
    ...
    let buildApp(app:IAppBuilder) =
        ...
        let twitterOptions = Twitter.TwitterAuthenticationOptions()
        twitterOptions.ConsumerKey <- config "TwitterConsumerKey"
        twitterOptions.ConsumerSecret <- config "TwitterConsumerSecret"
        twitterOptions.BackchannelCertificateValidator <- CertificateSubjectKeyIdentifierValidator(ids)

        // let facebookOptions = Facebook.FacebookAuthenticationOptions()
        // facebookOptions.Scope.Add("email")
        // facebookOptions.AppId <- config "FacebookAppId"
        // facebookOptions.AppSecret <- config "FacebookAppSecret"

        let fsOptions = FileServerOptions()
        ...
        fsOptions.FileSystem <- PhysicalFileSystem(config "StaticFileRoot")

        let userManager() = new PlUserManager(new PlUserStore())
        let cookieOptions = CookieAuthenticationOptions()
